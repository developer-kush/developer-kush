...
export default function Navbar(props:any){
  ...
  return (
    <div id="NAVBAR" className={`font-quicksnad sticky z-10 top-0 flex backdrop--md justify-between w-full h-20 text-white bg-gradient-to-b from-black to-transparent b0 -mb-20 `}>
      ...
      <Link href='/#INTRO' className={'flex items-center ml-6 text-3xl sm:ml-20 font- font-bebas '}>
        {
          ['K','U','S','H','A','G','R','A',' ','A','G','A','R','W','A','L'].map((i,idx)=>{
            return <span key={idx} className={`flex items-center h-20 transition-all duration-100 ease-in hover:-translate-y-1 ${colorlist[idx%3]}`}>{i==' '?<div className='px-1 text-transparent'>.</div>:i}</span>
          })
        }
      ...
      <div className='flex items-center text-[#cccccc] mx-2 sm:mr-20 font-quicksand font-bold '>
        {/* <div className='hidden transition-allduration-200 ease-in hover:text-white'>
